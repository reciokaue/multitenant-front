  ...
  "inline-flex items-center cursor-pointer justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-[color,box-shadow] disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      variant: {
        ...
        list: "hover:bg-accent hover:text-accent-foreground justify-start gap-2 select-none rounded-sm px-2 py-1.5 text-sm outline-none data-[disabled=true]:pointer-events-none aria-selected:bg-accent aria-selected:text-accent-foreground data-[disabled=true]:opacity-50"
      },
      size: {
      ...
      detail: {
        active: "border border-blue-500 bg-blue-200"
      }
    },
    defaultVariants: {
      variant: "default",
...
function Button({
  ...
  return (
    <Comp
      data-slot="button"
      className={cn(buttonVariants({ variant, size, detail, className }))}
      {...props}
    />
  )
}
